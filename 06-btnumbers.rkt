...
(define (btn-height bt)
  (match bt
    [(leaf) 0]
    [(node n left right) (if (> (+ 1 (btn-height left)) (+ 1 (btn-height right)))
                              (+ 1 (btn-height left))
                              (+ 1 (btn-height right))
                              )
                         
     ;;Note binary tree means each has at most two children
                         ]
    )
  )
...
(module+ test
  ...
  (check-equal? (btn-height (node 5 (node 1 (leaf) (leaf)) (node 1 (node 1 (leaf) (leaf)) (leaf)))) 3)
...
(define (btn-contains? bt n)
  (match bt
    [(leaf) #f]
    [(node q left right) (if (= q n) #t (or (btn-contains? left n) (btn-contains? right n)))]
    )
  )
