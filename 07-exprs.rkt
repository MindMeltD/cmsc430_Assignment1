...
(module+ test
  ...
  (check list-set-equal? (expr-lambda-vars (sexpr->expr '((lambda (x) y) 123))) '(x)))
...
(define (expr-free-vars e)
  (match e
    ...
    [(Lam x e) (remove x (expr-free-vars e))]
    )
  )
...
(module+ test
  ...
  (check list-set-equal? (expr-free-vars (sexpr->expr '((lambda (x) y) 123))) '(y))
